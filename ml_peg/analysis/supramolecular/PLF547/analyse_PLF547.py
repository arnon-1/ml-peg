...
@cache
def charged() -> list[bool]:
    """
    Get whether each system is charged.

    Returns
    -------
    list[bool]
        Whether each system is charged.
    """
    return struct_info()["charged"]
...
def charged_mae(interaction_energies) -> dict[str, float]:
    ...
    charged_indices = [i for i, is_charged in enumerate(charged()) if is_charged]
...
def neutral_mae(interaction_energies) -> dict[str, float]:
    ...
    """
    # Get charges for filtering
    charged_indices = [i for i, is_charged in enumerate(charged()) if not is_charged]
