...
def indices() -> list[int]:
    ...
    -------
    list[int]
        System indices, starting from 1.
    """
    return list(range(1, len(struct_info()["complex_charge"]) + 1))
...
@plot_parity(
    ...
    hoverdata=lambda: {
        "System": indices(),
        "Complex Atoms": struct_info()["counts"],
...
def s30l_charged_mae(interaction_energies) -> dict[str, float]:
    ...
    """
    charged_indices = [
        i for i, charge in enumerate(struct_info()["complex_charge"]) if charge != 0
    ]
